...
use clvm_to_arm_emulate::emu_stub::{start_stub, run_stub};
...
fn run_emu_stub(args: &Args) -> Result<(), String> {
    ...
    let symbols =
        if let Some(symbols) = &args.symbols {
            ...
            let result: HashMap<String, String> = serde_json::from_str(&sym_raw).map_err(|e| format!("{e:?}"))?;
            result
        } else {
            HashMap::default()
        };
    ...
    let (addr, connection) = start_stub(args.port.map(|p| p as u16)).map_err(|_e| "error starting stub".to_string())?;
    println!("{addr:?}");
